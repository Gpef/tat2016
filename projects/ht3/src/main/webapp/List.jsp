...
<%
    ...
    Phonebook phonebook = (Phonebook) request.getAttribute(Attributes.PHONEBOOK);
    ...
    userMessage = jspParameters.get(Attributes.PARAM_ACTION_RESULT_LABEL);
%>
<table align="center" border="1" width="90%">
    ...
    <tr>
        <td colspan="6" align="center">
            <a href="<%=request.getContextPath()%>/?action=<%=Attributes.ACTION_ADD_PERSON%>">Добавить запись</a>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            <a href="<%=request.getContextPath()%>/?action=<%=Attributes.ACTION_EDIT_PERSON%>&id=<%=person.getId()%>">Редактировать</a>
        </td>
        <td>
            <a href="<%=request.getContextPath()%>/?action=<%=Attributes.ACTION_DELETE_PERSON%>&id=<%=person.getId()%>">Удалить</a>
        </td>
    </tr>
    <%
        }
